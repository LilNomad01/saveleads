...
import { Trash2, Loader2, ExternalLink, Users, MessageCircle } from 'lucide-react';
...
import { TelegramLead } from '@/hooks/useTelegramLeads';
import { toast } from 'sonner';

interface TelegramLeadsTableProps {
  leads: TelegramLead[];
  isLoading: boolean;
  onDelete?: (ids: string[]) => Promise<boolean>;
}

export function TelegramLeadsTable({ leads, isLoading, onDelete }: TelegramLeadsTableProps) {
  ...
  return (
    <div className="bg-card rounded-xl shadow-card border border-border overflow-hidden">
      ...
      {leads.length === 0 ? (
        <div className="p-8 text-center text-muted-foreground">Nenhum lead do Telegram encontrado.</div>
      ) : (
            ...
            <TableBody>
              {leads.map(lead => (
                <TableRow key={lead.id} className={selected.has(lead.id) ? 'bg-muted/50' : ''}>
                  ...
                  <TableCell className="text-muted-foreground">{lead.username ? `@${lead.username}` : '—'}</TableCell>
                  <TableCell>
                    <Badge variant="outline">{lead.tipo || 'grupo'}</Badge>
                  </TableCell>
                  <TableCell>
                    {lead.membros ? (
                      <span className="flex items-center gap-1">
                        <Users className="h-3 w-3" />
                        {lead.membros.toLocaleString()}
                      </span>
                    ) : '—'}
                  </TableCell>
                  <TableCell>{lead.categoria || '—'}</TableCell>
                  <TableCell className="max-w-[200px] truncate text-sm text-muted-foreground">{lead.descricao || '—'}</TableCell>
                  <TableCell>
                    {lead.link ? (
                      <a href={lead.link} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1">
