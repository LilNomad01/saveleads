  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-700 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 bg-slate-800 border-b border-slate-700">
        ...
        <span className="text-sm font-medium text-slate-200">Console de Extração</span>
        {isExtracting && (
          <span className="ml-auto flex items-center gap-2">
            <span className="h-2 w-2 bg-green-400 rounded-full animate-pulse" />
            <span className="text-xs text-green-400">Extraindo...</span>
          </span>
        )}
      </div>
      ...
      <div 
        ref={scrollRef}
        className="h-[200px] overflow-y-auto p-4 font-mono text-sm space-y-1"
      >
        {logs.length === 0 ? (
          <div className="text-slate-500 text-center py-8">
            <Terminal className="h-8 w-8 mx-auto mb-2 opacity-50" />
            <p>Aguardando início da extração...</p>
          </div>
        ) : (
          logs.map((log) => {
            ...
            return (
              <div key={log.id} className="flex items-start gap-2 animate-fadeIn">
                <Icon className={cn('h-4 w-4 mt-0.5 flex-shrink-0', colorMap[log.tipo])} />
